...
def test_transfer_with_existing_accounts():
    ...
    tx = agr4bs.Transaction("account_0", "account_1", 0, value=100)
    vm = agr4bs.VM()
...
def test_transfer_with_insuficient_value():
    """
        Test that the VM returns the revert reason when a transfer is
        attempted with insuficient balance
    """
    ...
    tx = agr4bs.Transaction("account_1", "account_0", 0, value=100)
    vm = agr4bs.VM()

    receipt = vm.process_tx(state.copy(), tx)

    assert receipt.reverted is True
    assert receipt.revert_reason == "VM: Invalid balance for transfer"
