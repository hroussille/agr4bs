...
def test_blockchain_process_empty_block_votes():
    """
    ...
    attestations = []
    reverted_blocks, appended_blocks = blockchain.process_block_votes(attestations)
    
    assert reverted_blocks == []
    assert appended_blocks == []

    assert blockchain.head == genesis
    
def test_blockchain_process_block_votes():
    """
        Test that blockchain.process_block_votes() behaves correctly

        process_block_votes() should set the new head of the blockchain
        according to the latest block votes received from the network
    """
    genesis = Block(None, None, 0, [])
    blockchain = Blockchain(genesis)
    ...
    attestations = [Attestation("agent0", 0, 0, 0, block.hash, "", "")]
    reverted_blocks, appended_blocks = blockchain.process_block_votes(attestations)
...
def test_blockchain_process_block_votes_reorg():
    """
        Test that blockchain.process_block_votes() behaves correctly
        when a reorg is needed
    """
    genesis = Block(None, None, 0, [])
    blockchain = Blockchain(genesis)
    ...
    # Simulate attestations for block2 : triggering a reorg
    attestations = [Attestation("agent0", 0, 0, 0, block2.hash, "", "")]
    reverted_blocks, appended_blocks = blockchain.process_block_votes(attestations)

    assert reverted_blocks == [block1]
    assert appended_blocks == [block2]
